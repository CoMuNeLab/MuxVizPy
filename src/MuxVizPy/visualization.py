...
def Visualize_EdgeColoredNet(net, n_nodes=30, centr=None, pos_idx="agg", azim=10, elev=20, n_pow=3):
    ...
    if centr is None:
        ...
        ranked = centr.sort_values("vers", ascending=False).head(n_nodes)
        top_nodes = ranked["phy nodes"].to_numpy()
        centrality = np.zeros(net.Nodes, dtype=np.float64)
        centrality[centr["phy nodes"].to_numpy()] = centr["vers"].to_numpy()
    else:
        centrality = np.asarray(centr)
        if centrality.ndim != 1 or centrality.shape[0] != net.Nodes:
            raise ValueError("centr must contain one value per physical node")
        top_nodes = np.argsort(centrality)[::-1][:n_nodes]

    neighbors_mask = np.isin(np.arange(net.Nodes), top_nodes)
    selected_nodes = np.flatnonzero(neighbors_mask)
    sizes = np.power(np.exp(centrality[selected_nodes]), n_pow)
    gf_list = []
    ...
    if pos_idx=="agg":
        ...
        positions = gtdraw.sfdp_layout(gf_list[pos_idx]).get_2d_array([0,1])
